...
for iter = 1, modelParams.numEpochs do
    for convId = 1, numThreads do
        ...
        local ques = torch.Tensor(30, maxRounds):zero() -- right-aligned, answerer input, 1st word to ? token
        local quesIn = torch.Tensor(30, maxRounds):zero()
        local quesOut = torch.Tensor(30, maxRounds):zero()
        -- local ansIn = torch.Tensor(20, maxRounds):zero()
        local hist = torch.Tensor(40, maxRounds):zero()

        hist[{{40 - batch['hist']:size(3) + 1, 40}, {1, numSLRounds+1}}] = batchHist[{{},{1, numSLRounds+1}}];
        quesIn[{{1, batch['question_in']:size(3)}, {1, numSLRounds}}] = questionIn[{{}, {1, numSLRounds}}];
        quesOut[{{1, batch['question_out']:size(3)}, {1, numSLRounds}}] = questionOut[{{}, {1, numSLRounds}}];
        ques[{{30 - batchQues:size(1) + 1, 30}, {1, numSLRounds}}] = batchQues[{{}, {1, numSLRounds}}]
        local thread = {}
        for round = numSLRounds+1, 10 do
        ...
        r_t = qModel:annealedReinforce({hist, imgFeats, quesIn, quesOut}, numSLRounds, convId)
        ...
        if convId % 10 == 0 then
            ...
            print(string.format('[%s][SL:%d/%d][Epoch:%.02f][Iter:%d][Q Loss: %.02f][Img Loss: %.02f][Neg R: %.06f][lr: %f]',
                                os.date(), numSLRounds, maxRounds, currentEpoch, convId, runningLoss, runningLossImgRL, 100 * meanReward,
                                            qModel.optims.learningRate))
        end

        if convId % 1000 == 0 then
            print('Evaluating')
